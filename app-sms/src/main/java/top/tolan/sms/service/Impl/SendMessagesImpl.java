...
import top.tolan.sms.constant.SMSRedisKeys;
...
@Slf4j
@Service
public class SendMessagesImpl implements ISendMessages {
     ...
     */
    @Override
    public SendResult sendVerificationCode(String phone, @NonNull SMSTemplates smsTemplates) {
        if (StringUtils.isBlank(phone))
            throw new IllegalArgumentException("手机号不能为空");
        // 检查限制
        this.checkLimit(phone);
        // 生成并缓存验证码
        String code = this.cacheVerificationCode(phone, smsTemplates.getType());
        log.info("生成的验证码为：{}, 短信类型为：{}", code, smsTemplates.getDesc());
        // 发送验证码
        return this.sendVerificationCode(phone, code, smsTemplates.getCode());
    }
    ...
    private void checkLimit(String phone) {
        ...
        Integer sendTimes = redisCache.getCacheObject(SMSRedisKeys.SEND_TIMES + phone);
