package top.tolan.auth.wechat.provider;
...
import top.tolan.auth.base.dto.WechatAuthDTO;
...
public class WechatCodeAuthProv implements AuthenticationProvider {
    ...
    public LoginUser appLogin(String code, LoginUser loginUser) {
        ...
        try {
            ...
            WechatAuthDTO wechatAuthDTO = JSON.parseObject(response, WechatAuthDTO.class);
            assert ObjectUtils.isNotEmpty(wechatAuthDTO);
            if (wechatAuthDTO.getErrcode() == null) {
                ...
                log.error("微信登录失败, 错误码: {}, 错误消息: {}", wechatAuthDTO.getErrcode(), wechatAuthDTO.getErrmsg());
                throw new ServiceException("微信登录失败: " + wechatAuthDTO.getErrmsg());
            }
        } catch (Exception e) {
